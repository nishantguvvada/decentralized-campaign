...
beforeEach(async () => {
    ...
    await factory.methods.createCampaign('100')
        .send({from: accounts[0], gas: '1000000'});
    [campaignAddress] = await factory.methods.getDeployedCampaigns().call() // campaignAddress = addresses[0]
...
describe('Campaigns', () => {
    ...
    it('processes requests', async () => {
        ...
        let balance = await web3.eth.getBalance(accounts[1]); // getBalance() returns string type
        balance = web3.utils.fromWei(balance, 'ether'); // convert to ether
        balance = parseFloat(balance); // convert string to float/decimal number
        console.log(balance);
        assert(balance > 104);
    });
});
